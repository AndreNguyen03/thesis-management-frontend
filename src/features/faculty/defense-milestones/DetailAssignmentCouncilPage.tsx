...
import { CouncilScoringTab } from './components/CouncilScoringTab'
import { useAppSelector } from '@/store'
import { Tabs, TabsContent, TabsList, TabsTrigger } from '@/components/ui/tabs'
import { ROLES } from '@/models'
import { cn } from '@/lib/utils'
import { Input } from '@/components/ui'
export default function DetailAssignmentCouncilPage() {
	...
	const { data: defenseMilestoneDetail, isLoading: isLoadingDefenseMilestone } =
		useGetDefenseMilestoneDetailByIdQuery({ milestoneTemplateId: templateId || '' }, { skip: !templateId })
	...
	const [statusFilter, setStatusFilter] = useState<'all' | 'published' | 'locked' | 'pending' | 'completed'>('all')
	// Filtered topics for AwaitingTopicTable
	const filteredAwaitingTopics =
		paginationTopicData?.data?.filter((topic: any) => {
			if (statusFilter === 'all') return true
			if (statusFilter === 'pending') return !topic.isPublished && !topic.isLocked && !topic.isCompleted
			if (statusFilter === 'published') return topic.isPublished
			if (statusFilter === 'locked') return topic.isLocked
	...
	return (
		<div className='container mx-auto space-y-6 p-6'>
			{/* Header */}
			{!isLoadingDefenseMilestone ? (
				<CouncilHeader council={council} period={defenseMilestoneDetail?.periodInfo!} />
			) : (
				<div className='flex h-full w-full items-center justify-center'>
					<Loader2 className='h-8 w-8 animate-spin text-primary' />
				</div>
			...
			<Tabs defaultValue='topics'>
								...
								<CardContent>
									...
									<AwaitingTopicTable
										...
										setSelectedTopics={(id: string) => {
											const selectedTopic = paginationTopicData?.data.find(
												(topic) => topic._id === id
											)
				...
				<TabsContent value='scoring'>
					<CouncilScoringTab councilId={councilId!} isFacultyBoard={user?.role === ROLES.FACULTY_BOARD} />
				</TabsContent>
			</Tabs>
			{/* Topics Table */}
